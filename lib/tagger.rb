...
#
# Modified by SDB to use a larger lexicon, with Medpost stuff- http://mmtx.nlm.nih.gov/MedPost_SKR.shtml
#
...
class Tagger
  ...
  def initialize()
    @lexicon = {}
    file = File.new('lib/lexicon_combined.txt', 'r')
    file.each_line {|line|
                      toks=line.split
  ...
  def get_tag_structure(array_of_tokens)
    
    token_pos_tags = self.getTags(array_of_tokens.join(' '))
    
    # put them together into some sort of useful data structure:
    tagged_tokens = []
    array_of_tokens.each_index do |i|
      token = array_of_tokens[i]
      tag = token_pos_tags[i]
      tagged_tokens << {:token => token, :tag => tag}
    end
